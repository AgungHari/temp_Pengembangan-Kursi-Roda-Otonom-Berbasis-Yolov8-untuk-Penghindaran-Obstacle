            ...
            if not any(boxes) or boxes == []:
                ...
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    if current_direction == Direction.BERHASIL_KE_KIRI:
                        arah = 'A\n'
                        cv2.putText(img, f"Agung LesgooKiri", (50,50 ), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 255), 2)
                        start_point = (arrow_start_x, arrow_start_y)
                        end_point = (arrow_start_x - 100, arrow_start_y)
                    elif current_direction == Direction.BERHASIL_KE_KANAN:
                        arah = 'E\n'
                        cv2.putText(img, f"Agung LesgooKanan", (50,50 ), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 255), 2)
                        start_point = (arrow_start_x, arrow_start_y)
                        end_point = (arrow_start_x + 100, arrow_start_y)
                    else:
                        arah = 'B\n'
                        cv2.putText(img, f"Agung LesgooMaju", (50,50 ), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 255), 2)
                        start_point = (arrow_start_x, arrow_start_y)
                        end_point = (arrow_start_x, arrow_start_y - 100)
                    s.send(arah.encode('utf-8'))
                    time.sleep(6)
                ...
                # Kirim arah sesuai dengan current_direction
                    # with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    #     s.connect((host, port))
